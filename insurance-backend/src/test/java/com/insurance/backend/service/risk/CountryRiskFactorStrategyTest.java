...
class CountryRiskFactorStrategyTest {

    @Mock
    private RiskFactorConfigurationRepository riskRepository;

    @InjectMocks
    private CountryRiskFactorStrategy strategy;
    ...
    @Test
    void resolveShouldReturnEmptyList_whenCountryIsNull() {
        Building building = mock(Building.class);
        City city = mock(City.class);
        ...
        when(county.getCountry()).thenReturn(null);
    ...
    @Test
    void resolveShouldQueryRepository_whenCountryExists() {
        Building building = mock(Building.class);
        City city = mock(City.class);
        County county = mock(County.class);
        ...
        when(country.getId()).thenReturn(42L);

        RiskFactorConfiguration cfg = mock(RiskFactorConfiguration.class);
        when(riskRepository.findByLevelAndReferenceIdAndActiveTrue(RiskLevel.COUNTRY, 42L))
        ...
        verify(riskRepository, times(1))
                .findByLevelAndReferenceIdAndActiveTrue(RiskLevel.COUNTRY, 42L);
