...
@Component
public class PoliciesByCountyReportStrategy implements PolicyReportStrategy {

    private final PolicyRepository policyRepository;

    public PoliciesByCountyReportStrategy(PolicyRepository policyRepository) {
        this.policyRepository = policyRepository;
    }

    @Override
    public PolicyReportType type() {
        return PolicyReportType.COUNTY;
    }

    @Override
    public List<PolicyAggregateResponse> generate(PolicyReportFilter filter) {
        return policyRepository.aggregateByCounty(
                filter.from(),
                filter.to(),
                filter.status(),
                filter.currency(),
