...
@Component
public class PoliciesByBrokerReportStrategy implements PolicyReportStrategy {

    private final PolicyRepository policyRepository;

    public PoliciesByBrokerReportStrategy(PolicyRepository policyRepository) {
        this.policyRepository = policyRepository;
    }

    @Override
    public PolicyReportType type() {
        return PolicyReportType.BROKER;
    }

    @Override
    public List<PolicyAggregateResponse> generate(PolicyReportFilter filter) {
        return policyRepository.aggregateByBroker(
                filter.from(),
                filter.to(),
                filter.status(),
                filter.currency(),
